<script>
  /**
   * Comment modal — overlay for adding a comment to a node.
   */
...
{#if visible && node}
  ...
  <div class="overlay" onclick={onclose}>
    <div class="modal" onclick={(e) => e.stopPropagation()}>
      ...
      <textarea
        bind:value={text}
        placeholder="What would you change about this component?"
        rows="3"
      ></textarea>
      <div class="actions">
...
<style>
  ...
  textarea {
    ...
    border-radius: 6px; color: var(--tx); padding: 8px;
  ...
  .cancel {
    ...
    background: var(--bdr); color: var(--tx-m); font-size: 13px; cursor: pointer;
  }
  .save {
    padding: 6px 16px; border-radius: 6px; border: none;
    background: var(--ac); color: white; font-size: 13px; cursor: pointer;
  }
