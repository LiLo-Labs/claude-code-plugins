<script>
   ...
   * Different options for nodes vs canvas background.
   */
  let { x = 0, y = 0, visible = false, node = null, isCanvas = false, onaction, onclose } = $props();
</script>
...
{#if visible}
  ...
  <div class="ctx" style="left: {x}px; top: {y}px" onclick={(e) => e.stopPropagation()}>
    {#if isCanvas}
      ...
      <div class="ctx-item" onclick={() => { onaction?.('new-node'); onclose?.(); }}>&#10010; New Node</div>
    {:else if node}
      <!-- Node menu -->
      <div class="ctx-header">{node.label}</div>
      <div class="ctx-sep"></div>
      <div class="ctx-item" onclick={() => { onaction?.('comment'); onclose?.(); }}>&#128172; Add Comment</div>
      <div class="ctx-item" onclick={() => { onaction?.('connect'); onclose?.(); }}>&#10132; Connect to...</div>
      <div class="ctx-sep"></div>
      ...
      <div class="ctx-item" onclick={() => { onaction?.('status-planned'); onclose?.(); }}>
        <span style="color: #3b82f6">&#9679;</span> Planned
      </div>
      <div class="ctx-sep"></div>
      <div class="ctx-item" onclick={() => { onaction?.('details'); onclose?.(); }}>&#128196; Details</div>
      <div class="ctx-item" onclick={() => { onaction?.('remove-from-tab'); onclose?.(); }}>&#10005; Remove from Tab</div>
    {/if}
  </div>
{/if}
